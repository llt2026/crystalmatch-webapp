...
exports.findReportInDatabase = findReportInDatabase;
exports.saveReportToDatabase = saveReportToDatabase;
exports.deleteUserReport = deleteUserReport;
exports.clearExpiredReports = clearExpiredReports;
 ...
 * 生成报告缓存键
 */
function generateCacheKey(userId, birthDate, currentDate, tier) {
 ...
 * 从数据库中查找报告
 */
async function findReportInDatabase(key) {
    try {
        // 首先尝试从内存缓存获取（如果启用）
 ...
 * 保存报告到数据库
 */
async function saveReportToDatabase(key, report, energyContext) {
    try {
        // 计算过期时间（默认1个月后）
        const now = new Date();
 ...
 * 清除用户的特定报告
 */
async function deleteUserReport(userId, reportMonth) {
    try {
        // 构建查询条件
        const where = { userId };
        if (reportMonth) {
 ...
 * 清除过期的报告缓存
 */
async function clearExpiredReports() {
    try {
        // 从数据库中删除过期的报告
 ...
 * 获取用户报告统计信息
 */
async function getUserReportStats(userId) {
    try {
        // 获取当前月份
        const now = new Date();
